      ...
      const finalData = await getOrPopulateSharedCache(
        ...
        async () => {
          ...
          if (!validatedPrice) {
            ...
            const stalePrice =
              toPositiveNumber(staleRescue?.currentPrice) ||
              toPositiveNumber(staleRescue?.price) ||
              toPositiveNumber(staleRescue?.regularMarketPrice) ||
              toPositiveNumber(staleRescue?.regularMarketPreviousClose) ||
              toPositiveNumber(staleRescue?.previousClose) ||
              toPositiveNumber(staleRescue?.close);

            if (staleRescue && stalePrice > 0 && staleRescue.timestamp) {
              ...
              if (policy.acceptable) {
                ...
                return {
                  ...
                  currentPrice: stalePrice,
                  price: stalePrice,
                  regularMarketPrice: stalePrice,
                  chosenPrice: stalePrice,
                  chosenPriceField: "STALE_CACHE_RESCUE",
